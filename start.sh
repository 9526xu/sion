...
#pid=`ps -C java  | grep -- 9090 | awk '{print $1}'`
  ...
  #kill -9 $pid
#fi
java $JAVA_OPTIONS -jar $PROJECT_HOME_SRC/${PROJECT_NAME}-core/target/$PROJECT_NAME.jar --spring.profiles.active=$SPRING_PROFILES_ACTIVE --spring.dubbo.protocol.port=$SPRING_DUBBO_PROTOCOL_PORT --server.port=$JETTY_PORT | tee $BUILD_LOG
running() {
